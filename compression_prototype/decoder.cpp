...
int main(int argc, char* argv[]) {
    ...
    const int frame_size   = br.readRaw<uint16_t>();
    const int hop_size     = br.readRaw<uint16_t>();
    const int sample_rate  = br.readRaw<uint32_t>();
    const float peak_ref   = br.readFloat();
    const float rms_ref    = br.readFloat();
    const int ktop_global  = br.readRaw<uint16_t>();
    const int qmag_bits    = br.readRaw<uint8_t>();
    const int qphase_bits  = br.readRaw<uint8_t>();
    const int threshold    = br.readRaw<uint16_t>();
    const int total_frames = br.readRaw<uint32_t>();
