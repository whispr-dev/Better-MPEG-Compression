...
int main(int argc, char* argv[]) {
    ...
    BitWriter bw;
    // metadata
    bw.writeBits((uint32_t)frame_size, 16);
    bw.writeBits((uint32_t)hop_size,   16);
    bw.writeBits((uint32_t)wav.sample_rate, 32);
    bw.writeFloat(peak);
    bw.writeFloat(rms);
    bw.writeBits((uint32_t)ktop, 16);
    bw.writeBits((uint32_t)qmag_bits, 8);
    bw.writeBits((uint32_t)qphase_bits, 8);
    bw.writeBits((uint32_t)threshold, 16);
    bw.writeBits((uint32_t)total_frames, 32);
